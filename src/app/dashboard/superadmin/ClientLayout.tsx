...
const navItems: NavItem[] = [
  ...
  { href: '/dashboard/superadmin/organizations', label: 'Organizations', icon: Building2 },
  { href: '/dashboard/superadmin/malls', label: 'Malls', icon: Store },
  { href: '/dashboard/superadmin/restaurants', label: 'Restaurants', icon: ShoppingBag, badge: 12 },
  { href: '/dashboard/superadmin/users', label: 'Users', icon: Users },
  { href: '/dashboard/superadmin/revenue', label: 'Revenue', icon: IndianRupee },
  { href: '/dashboard/superadmin/commission', label: 'Commission', icon: Percent },
  ...
  { href: '/dashboard/superadmin/system-health', label: 'System Health', icon: Activity },
  { href: '/dashboard/superadmin/feature-flags', label: 'Feature Flags', icon: Flag },
  { href: '/dashboard/superadmin/support', label: 'Support Tickets', icon: ScrollText },
  { href: '/dashboard/superadmin/settings', label: 'Global Settings', icon: Settings },
];
...
export function useSuperAdminDashboard() {
  ...
  if (!context) throw new Error('useSuperAdminDashboard must be used within SuperAdminLayout');
  return context;
}

const initialNotifications: Notification[] = [
  { id: '1', title: 'New Organization Signup', message: 'Foodie Group Inc. has registered on the platform', read: false, time: '5m ago', type: 'alert' },
  { id: '2', title: 'Payment Gateway Issue', message: 'Razorpay experiencing intermittent failures', read: false, time: '12m ago', type: 'alert' },
  { id: '3', title: 'System Update', message: 'Platform v2.5.1 deployment scheduled at 3 AM', read: true, time: '1h ago', type: 'system' },
  { id: '4', title: 'New User Milestone', message: 'Platform crossed 50,000 registered users', read: false, time: '2h ago', type: 'info' },
  { id: '5', title: 'Daily Revenue Report', message: "Global revenue today: ₹12,45,000", read: true, time: '3h ago', type: 'system' },
  { id: '6', title: 'Restaurant Pending Approval', message: '24 restaurants awaiting moderation', read: false, time: '4h ago', type: 'alert' },
];
  ...
  return (
    ...
    >
      <div className={cn('flex h-16 items-center border-b px-4', collapsed ? 'justify-center' : 'justify-between')}>
        <Link href="/dashboard/superadmin" className={cn('flex items-center gap-2.5', collapsed && 'justify-center')}>
        ...
        <div className={cn('flex items-center gap-3', collapsed && 'flex-col')}>
          ...
          {!collapsed && (
            <div className="flex-1 min-w-0">
              ...
              <p className="truncate text-xs text-muted-foreground">Platform Administration</p>
            </div>
          )}
        </div>
      </div>
        ...
        <>
          ...
          >
            <div className="flex h-16 items-center justify-between border-b px-4">
              <Link href="/dashboard/superadmin" className="flex items-center gap-2.5" onClick={onClose}>
                <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-primary shadow-sm">
                  <Shield className="h-5 w-5 text-white" />
                </div>
                <span className="font-display text-lg font-bold tracking-tight">
...
function Header({ onMenuClick }: { onMenuClick: () => void }) {
  ...
  const { notifications, unreadCount, markAllAsRead } = useSuperAdminDashboard();
    ...
    <header className="sticky top-0 z-30 flex h-16 items-center gap-4 border-b bg-background/80 px-4 backdrop-blur-xl sm:px-6">
      ...
      <div className="hidden md:flex md:flex-1">
        <div className="relative w-full max-w-md">
          ...
          <Input
            placeholder="Search organizations, malls, users..."
            value={searchQuery}
      ...
      <div className="flex flex-1 items-center justify-end gap-2">
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-56">
            ...
            <DropdownMenuSeparator />
            <DropdownMenuItem onClick={() => router.push('/dashboard/superadmin/settings')}>
...
export default function SuperAdminClientLayout({ children }: { children: ReactNode }) {
